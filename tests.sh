...
@test "read int32 - v3" {
  ...
  echo "Command: $ZARR_CLI --array_path=$ZARR_CONFORMANCE_DATA/int32_v3.zarr" >&3
  echo "Exit status: $status" >&3
  echo "Output: $output" >&3
  [ "$status" -eq 0 ]
}
...
@test "read float64 - v2" {
  ...
  echo "Command: $ZARR_CLI --array_path=$ZARR_CONFORMANCE_DATA/float64.zarr" >&3
  echo "Exit status: $status" >&3
  echo "Output: $output" >&3
  [ "$status" -eq 0 ]
}
...
@test "read float32 - v2" {
  ...
  echo "Command: $ZARR_CLI --array_path=$ZARR_CONFORMANCE_DATA/float32.zarr" >&3
  echo "Exit status: $status" >&3
  echo "Output: $output" >&3
  [ "$status" -eq 0 ]
}
...
@test "read bool - v2" {
  ...
  echo "Command: $ZARR_CLI --array_path=$ZARR_CONFORMANCE_DATA/bool.zarr" >&3
  echo "Exit status: $status" >&3
  echo "Output: $output" >&3
  [ "$status" -eq 0 ]
}